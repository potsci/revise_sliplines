...
function write_statistic(subfolder,file,output_file,T,Ss,slip_name,devang,line2,index)
% function write_statistic(subfolder,file,Rep_counts,output_file,T,Ss,slip_name,devang,line2)
    if ~isfile([subfolder{1}, '\', output_file, '.xlsx'])
        %%
        headline=["image_index","index", "phi1", 'Phi', 'phi2','slip_sys_no','slip_name','devang','point1_x','point1_y','point2_y','point2_y'];
        writematrix(headline,[subfolder{1},'\', output_file, '.xlsx'],'Sheet',1,'Range','A1');
         writematrix(headline,[subfolder{1},'\', output_file, '.csv'],'Delimiter',';');
    end
    %%
    disp(':)')
    % Import the data
...
%%
    ...
    writetable(output_current,[subfolder{1},'\',output_file, '.csv'],'Delimiter',';','WriteMode','Append');
end
